...
const state = { lang: localStorage.getItem('audio-integrity-lang') || (navigator.language.startsWith('ru') ? 'ru' : 'en'), socket:null, reconnectTimer:null, connectionKey:'connecting', lastCounters:null, scanStatus:null, historyRuns:[], resultsPage:0, resultsPageSize:[25,50,100].includes(savedPageSize) ? savedPageSize : 25, resultsRequestId:0, performanceEstimator:AudioIntegrityMetrics.createPerformanceEstimator() };
...
function renderStatus(scan) {
    ...
    const performance = state.performanceEstimator.update(scan);
    $('#live-throughput').textContent = performance.ready && performance.validationBytesPerSecond > 0 ? `${formatBytes(performance.validationBytesPerSecond)}/s` : '—';
    $('#live-eta').textContent = AudioIntegrityMetrics.formatDuration(running ? performance.etaSeconds : NaN, state.lang);
    const log = scan.log || [];
